...
describe("PionerV1Close Signatures Contract", function () {
  ...
  beforeEach(async function () {
    ...
    await pionerV1.connect(owner).setContactAddress(
      _daiAddress ,
      _min_notional ,
      _frontend_share ,
      _affiliation_share ,
    ...
    await pionerV1Oracle.deployBOraclePion( _x, _parity,_maxConfidence,_assetHex,_maxDelay, _precision, _imA,_imB,_dfA,_dfB,_expiryA,_expiryB,_timeLock,_cType );
  ...
  it("openMM + closeMM warpers", async function () {
    ...
    const openQuoteSignature = await addr1.signTypedData(domainOpen, openQuoteSignType, openQuoteSignValue);
  ...
  const bOracleSignValue = {
    ...
    await pionerV1Warper.connect(addr2).warperOpenQuoteMM(
      bOracleSignValue,
  ...
  const signCloseQuote = await addr1.signTypedData(domainClose, OpenCloseQuoteType, openCloseQuoteValue);

    await pionerV1Warper.connect(addr2).warperCloseLimitMM( openCloseQuoteValue, signCloseQuote);
