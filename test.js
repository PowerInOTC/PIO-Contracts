...
async function closeQuoteSign(signer, closeQuoteSign) {
  const signatureCloseQuoteSign = await signer.signTypedData(domainWarper, closeQuoteSign, acceptCloseQuoteValue);
...
async function sendTx() {
  const txPromises = transactionQueue.map(async ({ chainId, signerIndex, functionName, args }) => {
    ...
    if (namespace === 'v1') {
      ...
      const contractInstance = configs[chainId].contracts[contractName].instance;
...
async function main() {
  ...
  addTx('sonic', 0, 'v1.approve', configs.sonic.contracts.pionerV1Compliance.address, mintAmount);
  addTx('sonic', 1, 'v1.approve', configs.sonic.contracts.pionerV1Compliance.address, mintAmount);
  await sendTx();
  ...
  addTx('sonic', 0, 'sendTransaction', { to: signers['sonic'][1].address, value: gasAmount });
  await sendTx();
  ...
  // from API
  const openQuoteSignature = await openQuoteSign(signers['sonic'][1], openQuoteSignValue);
  bOracleSignValue.signatureHashOpenQuote = openQuoteSignature;
  const signaturebOracleSign = await bOracleSign(signers['sonic'][1], bOracleSignValue);

  //Hedger
  const _acceptPrice = ethers.parseUnits("50", 18);
  await configs.sonic.contracts.pionerV1Warper.instance.connect(signers['sonic'][2]).warperOpenQuoteMM(
    bOracleSignValue,
  ...
  const signatureCloseQuoteSign = await bOracleSign(signers['sonic'][1], closeQuoteValue);
  ...
  await configs.sonic.contracts.pionerV1Warper.instance.connect(signer2).warperCloseLimitMM( closeQuoteValue, signatureCloseQuoteSign);
